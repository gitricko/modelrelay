...
import { anthropicToOpenAI, openaiToAnthropic, buildAnthropicModelsList, mapModel, AnthropicStreamTransformer, createAnthropicStreamTransformer } from '../lib/anthropic-adapter.js'
...
describe('anthropic adapter — anthropicToOpenAI', () => {
  ...
  it('handles thinking block without text content', () => {
    ...
    // No text content → content should be empty string
  ...
  it('converts tool_use blocks to OpenAI tool_calls', () => {
    ...
    assert.equal(result.messages[1].tool_calls.length, 1)
    assert.equal(result.messages[1].tool_calls[0].id, 'tu_001')
    assert.equal(result.messages[1].tool_calls[0].function.name, 'calculator')
    assert.equal(result.messages[1].tool_calls[0].function.arguments, '{"a":2,"b":2}')
  })
  ...
  it('sanitizes tool names containing special characters', () => {
    const result = anthropicToOpenAI({
      model: 'claude-sonnet-4-5',
      max_tokens: 100,
...
describe('anthropic adapter — openaiToAnthropic', () => {
  ...
  it('converts reasoning_content to Anthropic thinking block', () => {
    const result = openaiToAnthropic({
      id: 'chatcmpl-123',
  ...
  it('converts tool_calls to tool_use blocks', () => {
    ...
    assert.equal(result.content[0].id, 'call_001')
    assert.equal(result.content[0].name, 'calculator')
    assert.deepEqual(result.content[0].input, { a: 2, b: 2 })
    assert.equal(result.stop_reason, 'tool_use')
  })

  it('reverses tool name mapping', () => {
...
describe('anthropic adapter — AnthropicStreamTransformer', () => {
  it('emits correct SSE sequence for plain text chunks', () => {
    const t = new AnthropicStreamTransformer('claude-sonnet-4-5')
    const e1 = t.pushChunk({ choices: [{ delta: { role: 'assistant', content: 'Hello' }, finish_reason: null }] })
    assert.ok(e1.some(e => JSON.parse(e).type === 'message_start'))
    assert.ok(e1.some(e => JSON.parse(e).type === 'content_block_start'))

    const e2 = t.pushChunk({ choices: [{ delta: { content: ' world' }, finish_reason: null }] })
    assert.ok(e2.some(e => JSON.parse(e).type === 'content_block_delta'))

    const e3 = t.pushChunk({ choices: [{ delta: {}, finish_reason: 'stop' }] })
    const types = e3.map(e => JSON.parse(e).type)
  ...
  it('handles reasoning_content in streaming chunks', () => {
    const t = new AnthropicStreamTransformer('claude-sonnet-4-5')
    // First chunk: message_start + content_block_start
    const e1 = t.pushChunk({ choices: [{ delta: { role: 'assistant', content: null, reasoning_content: 'Thinking' }, finish_reason: null }] })
    const e1Types = e1.map(e => JSON.parse(e).type)
    assert.ok(e1Types.includes('message_start'))
    assert.ok(e1Types.includes('content_block_start'))

    // Second chunk: content_block_delta
    const e2 = t.pushChunk({ choices: [{ delta: { content: null, reasoning_content: ' further' }, finish_reason: null }] })
    const e2Types = e2.map(e => JSON.parse(e).type)
    assert.ok(e2Types.includes('content_block_delta'))

    // Final chunk: content_block_stop + message_delta + message_stop
    const e3 = t.pushChunk({ choices: [{ delta: {}, finish_reason: 'stop' }] })
    const e3Types = e3.map(e => JSON.parse(e).type)
  ...
  it('handles tool_calls in streaming chunks', () => {
    const t = new AnthropicStreamTransformer('claude-sonnet-4-5')
    // First tool call chunk
    const e1 = t.pushChunk({
      choices: [{
        delta: { role: 'assistant', content: null, tool_calls: [{ index: 0, id: 'call_1', type: 'function', function: { name: 'calc', arguments: '' } }] },
        finish_reason: null,
      }],
    })
    const e1Types = e1.map(e => JSON.parse(e).type)
    ...
    const e2 = t.pushChunk({
      choices: [{
        delta: { tool_calls: [{ index: 0, function: { arguments: '{"a":' } }] },
        finish_reason: null,
      }],
    })
    const e3 = t.pushChunk({
      choices: [{
  ...
  it('does not emit events after done', () => {
    const t = new AnthropicStreamTransformer('claude-sonnet-4-5')
    t.pushChunk({ choices: [{ delta: { content: 'Hi' }, finish_reason: 'stop' }] })
    const afterDone = t.pushChunk({ choices: [{ delta: { content: 'More' }, finish_reason: null }] })
    assert.equal(afterDone.length, 0)
  })
})

describe('anthropic adapter — buildAnthropicModelsList', () => {
  it('returns expected model IDs', () => {
...
describe('anthropic adapter — mapModel', () => {
  it('maps claude models to auto-fastest by default', () => {
  ...
  it('defaults to auto-fastest for unknown model', () => {
    assert.equal(mapModel('unknown-model'), 'auto-fastest')
  })

  it('defaults to auto-fastest for null/undefined', () => {
