...
export function normalizeConfigShape(config) {
  ...
  // OAuth credentials block (browser-authenticated providers like Antigravity)
  if (!base.oauth || typeof base.oauth !== 'object' || Array.isArray(base.oauth)) {
    base.oauth = {}
  }
...
// Mirrors PicoClaw's pkg/auth/store.go pattern but stores inside ~/.modelrelay.json
// under config.oauth rather than a separate auth.json file.
 ...
 * @property {string} authMethod    'browser'
 */

/**
 * Read an OAuth credential for a provider from config.
 * @param {object} config
 * @param {string} provider  e.g. 'antigravity'
 * @returns {OAuthCredential|null}
 */
export function getOAuthCredential(config, provider) {
  return config?.oauth?.[provider] || null
}

/**
 * Write an OAuth credential for a provider into config and persist to disk.
 * @param {object} config  (mutated in place)
 * @param {string} provider  e.g. 'antigravity'
 * @param {OAuthCredential} cred
 */
export function setOAuthCredential(config, provider, cred) {
  if (!config.oauth) config.oauth = {}
  config.oauth[provider] = cred
  saveConfig(config)
}

/**
 * Remove an OAuth credential for a provider from config.
 * @param {object} config  (mutated in place)
 * @param {string} provider
 */
export function deleteOAuthCredential(config, provider) {
  if (config.oauth) delete config.oauth[provider]
  saveConfig(config)
}
