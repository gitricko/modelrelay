...
export async function runOnboard(port = 7352) {
  ...
  for (const providerKey of PROVIDER_ORDER) {
    ...
    // ── Antigravity uses browser OAuth, not a paste-key ──────────────────────
    if (providerKey === 'antigravity') {
      const existingOAuth = config.oauth?.antigravity
      console.log(`- ${providerName} (${providerKey})`)
      console.log('  note: Uses browser OAuth (Google Cloud Code Assist) — no API key needed')
      if (existingOAuth?.email) {
        console.log(`  current: logged in as ${existingOAuth.email}`)
      } else {
        console.log('  current: not logged in')
        console.log(`  login:   visit http://127.0.0.1:${port}/auth/antigravity/start while the router is running`)
      }
      console.log('')
      continue
    }
