...
export function filterModelsByRequested(results, requestedModel, canonicalizeFn) {
  ...
  // Treat any provider-prefixed auto-fastest (e.g. 'openai/auto-fastest', 'router/auto-fastest',
  // 'modelrelay/auto-fastest') the same as the bare 'auto-fastest' wildcard.
  const effectiveId = typeof canonicalizeFn === 'function'
    ? (canonicalizeFn(requestedModel).unprefixed || requestedModel)
    : requestedModel
  if (effectiveId === 'auto-fastest') return results
