...
const PROJECT_ID_URL = 'https://cloudcode-pa.googleapis.com/v1internal/projects/-:listPublicProjects?pageSize=1'
const ANTIGRAVITY_MODELS_URL = (projectId) =>
  `https://cloudcode-pa.googleapis.com/v1internal/projects/${projectId}/locations/-/publishers/google/models`
...
export async function fetchProjectId(accessToken) {
  const res = await fetch(PROJECT_ID_URL, {
    headers: {
  ...
  const data = await res.json()
  // Expected: { projects: [{ name: "projects/...", projectId: "..." }] }
  const project = data?.projects?.[0]
  if (!project) throw new Error('No Cloud Code Assist project found for this account')
  // Extract just the project ID from "projects/my-project-id"
...
export async function fetchAntigravityModels(accessToken, projectId) {
  const res = await fetch(ANTIGRAVITY_MODELS_URL(projectId), {
    headers: {
      Authorization: `Bearer ${accessToken}`,
      'Content-Type': 'application/json',
    },
